...
namespace TenantAndLeaseManagement.Domain.Entities
{
    public class LeaseAgreement
    {
        ...
        public Guid IndividualUnitId { get; private set; }
        public double MonthlyRent { get; private set; }
        protected LeaseAgreement() { }
        private LeaseAgreement(string tenantName, string ownerName, DateTime creationDate, DateTime terminationDate, Guid individualUnitId, double monthlyRent)
        {
        ...
        public static LeaseAgreement Create(string tenantName, string ownerName, DateTime creationDate, DateTime terminationDate, Guid individualUnitId, double monthlyRent)
        {
            ...
            return new LeaseAgreement(tenantName, ownerName, creationDate, terminationDate, individualUnitId, monthlyRent);
        }
        public static LeaseAgreement Renew(LeaseAgreement existingLease, DateTime newTerminationDate, double newMonthlyRent)
        {
            ...
            return new LeaseAgreement(existingLease.TenantName, existingLease.OwnerName, existingLease.CreationDate, newTerminationDate, existingLease.IndividualUnitId, newMonthlyRent);
        }
        public static LeaseAgreement Terminate(LeaseAgreement existingLease)
        {
            if (existingLease == null)
                throw new ArgumentNullException(nameof(existingLease), "Existing lease cannot be null.");
            return new LeaseAgreement(existingLease.TenantName, existingLease.OwnerName, existingLease.CreationDate, DateTime.Now, existingLease.IndividualUnitId, existingLease.MonthlyRent);
        }
    }
}
